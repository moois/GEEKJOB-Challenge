 ...
 * @author mooni
 */
public class dbc2 extends HttpServlet {
...
//前回の課題「テーブルの作成とinsert」で作成したテーブルに自由なメンバー情報を格納する処理を構築してください。
        ...
        try{
            ...
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/challenge_db","root","");
                //SQL文の入力。処理を行わせる。
                st = con.prepareStatement("insert into user(userID,name,tell,age,birthday)values(6,'こばちゃん','090-1122-3344',22,'1994-07-13')");
                //実行する
                st.executeUpdate();
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet dbc2</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet dbc2 at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
